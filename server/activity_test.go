...
func TestUserAccountActorDoesNotFetchProfile(t *testing.T) {
	...
	req := httptest.NewRequest("GET", "https://oginstagram.com/users/cutie_street_", nil)
	res := a.handleUserAccount(req, "cutie_street_")
	if res.status != 200 {
	...
	if actor["name"] != "cutie_street_" || actor["preferredUsername"] != "cutie_street_" {
		t.Fatalf("actor should use username-only fallback, got %#v", actor)
	}
	if actor["url"] != "https://oginstagram.com/users/cutie_street_" {
		t.Fatalf("actor url = %v, want local actor URL", actor["url"])
	}
	if actor["inbox"] != "https://oginstagram.com/users/cutie_street_/inbox" {
		t.Fatalf("actor inbox = %v", actor["inbox"])
	}
	if actor["outbox"] != "https://oginstagram.com/users/cutie_street_/outbox" {
		t.Fatalf("actor outbox = %v", actor["outbox"])
	}
	if actor["followers"] != "https://oginstagram.com/users/cutie_street_/followers" {
		t.Fatalf("actor followers = %v", actor["followers"])
	}
	if actor["following"] != "https://oginstagram.com/users/cutie_street_/following" {
...
func TestUserActivityCollectionsAreOrderedCollections(t *testing.T) {
	a := &App{cfg: Config{BrandName: "OGInstagram", Port: 8080}}
	req := httptest.NewRequest("GET", "https://oginstagram.com/users/cutie_street_/outbox", nil)
	res := a.handleUserCollection(req, "cutie_street_", "outbox")
	if res.status != 200 {
		t.Fatalf("status = %d, want 200", res.status)
	}
	...
	if coll["id"] != "https://oginstagram.com/users/cutie_street_/outbox" {
